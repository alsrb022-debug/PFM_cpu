...
# ============================================================
def dominant_gid_map_raw(phi, gid):
    """
    Return dominant raw gid map (without +1 offset).
    shape: (im, jm, km)
    """
    im, jm, km, nmax = phi.shape
    out = np.full((im, jm, km), EMPTY_ID, dtype=np.int32)
...
def extract_phi_profile_1d(phi, gid):
    """
    Extract 1D phi profiles by raw grain id.
    Returns dict: gid -> profile array
    """
    im, jm, km, nmax = phi.shape
    ...
    for i in range(im):
        for s in range(nmax):
            g = gid[i, 0, 0, s]
            if g != EMPTY_ID:
                valid_gids.add(int(g))

    profiles = {}
    for gtarget in sorted(valid_gids):
        prof = np.zeros(im, dtype=np.float64)
        for i in range(im):
            val = 0.0
            for s in range(nmax):
                if gid[i, 0, 0, s] == gtarget:
                    val = phi[i, 0, 0, s]
                    break
            prof[i] = val
        profiles[gtarget] = prof
...
# Visualization
# ============================================================
def visualize_1d(phi, gid, step, time, out_dir, stem):
    profiles = extract_phi_profile_1d(phi, gid)
    im = phi.shape[0]
...
def visualize_2d(phi, gid, step, time, out_dir, stem):
    gmap = dominant_gid_map_raw(phi, gid)[:, :, 0]
...
def visualize_3d(phi, gid, step, time, out_dir, stem):
    gmap = dominant_gid_map_raw(phi, gid)
...
def visualize_pf(phi, gid, step, time, out_dir, stem):
    ndim = detect_ndim_from_shape(phi)

    if ndim == 1:
        visualize_1d(phi, gid, step, time, out_dir, stem)
    elif ndim == 2:
...
def benchmark_plane(phi, gid, params, step, time, out_dir, stem):
    ...
    # eta = half interface thickness [m]
    eta = params["xi_in"] * dx
...
def benchmark_circle(all_files, params, out_dir):
    _, _, _, emm = compute_pf_coefficients(params)
...
# Process helpers
# ============================================================
def process_one_snapshot(snap_path, out_dir):
    phi, gid, step, time = load_snapshot(snap_path)
    stem = snap_path.stem
    visualize_pf(phi, gid, step, time, out_dir, stem)
    print(f"Processed plot: {snap_path.name}")


def process_all_snapshots(all_files, out_dir):
    for snap_path in all_files:
        process_one_snapshot(snap_path, out_dir)
...
def process_pf(input_file="input.txt", in_dir="p_out", out_dir="fig_pf", snapshot=None):
    ...
    process_all_snapshots(all_files, out_dir)
